package kantczak.imagelabelsproject.myDrawView;
...
public class MyDrawView extends View {
    ...
    public MyDrawView(Context context, AttributeSet attrs, int defStyle) {
        ...
        AllPointsToChange = new ArrayList<>();
        option = null;
        mask = null;
    }
    ...
    public void setDrawable(Drawable drawable,int displayWidth) {
        startImageWidth = drawable.getIntrinsicWidth();
        startImageHeight = drawable.getIntrinsicHeight();
        this.image = resizeDrawableToWidth(displayWidth,startImageWidth, startImageHeight, drawable);
        image.setBounds(0, 0, image.getIntrinsicWidth(), image.getIntrinsicHeight());
    ...
    public void startDraw(boolean start) {
        this.isDraw = start;
    }

    public void blockPicture(boolean block) {
        this.isBlock = block;
    }
    ...
    private Drawable resizeDrawableToWidth(int displayWidth,int drawableWidth, int drawableHeight, Drawable drawable) {
        Bitmap b = ((BitmapDrawable) drawable).getBitmap();
        Bitmap bitmapResized;
        if (drawableWidth > 500){
            float height = ((float)drawableHeight / ((float)drawableWidth / (float)displayWidth));
            bitmapResized = Bitmap.createScaledBitmap(b, displayWidth, ((int)height), false);
        }else{
            float height = ((float)drawableHeight * ((float)displayWidth/(float)drawableWidth));
            bitmapResized = Bitmap.createScaledBitmap(b, displayWidth, ((int)height), false);
        }
